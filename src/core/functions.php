...
function dmopress_get_option($option){
	if($option){
		...
		if(in_array($option, $optionRef[$option])){
...
function dmopress_available_modules(){
	$available_modules = array(
		array(
			'slug' => 'accepted-payment-types',
			'label' => __('Accepted Payment Types','dmopress_textdomain'),
			'default' => 'enabled',
			'description' => __('Track accepted payment types at each of your Places.', 'dmopress_textdomain'),
		),
		array(
			'slug' => 'tribe-events-calendar',
			'label' => __('Events Calendar and Events Calendar Pro', 'dmopress_textdomain'),
			'default' => 'enabled',
			'description' => __('Integration with the popular Events Calendar and Events Calendar Pro plugins.', 'dmopress_textdomain'),
		),
		array(
			'slug' => 'map',
			'label' => __('Google Maps','dmopress_textdomain'),
			'default' => 'enabled',
			'description' => __('Display your Places on stylized, interactive Google Maps.', 'dmopress_textdomain'),
		),
		array(
			'slug' => 'hours-of-operation',
			'label' => __('Hours of Operation','dmopress_textdomain'),
			'default' => 'enabled',
			'description' => __('Track the hours of operation for your Places.', 'dmopress_textdomain'),
		),
		array(
			'slug' => 'tripadvisor',
			'label' => __('TripAdvisor','dmopress_textdomain'),
			'default' => 'enabled',
			'description' => __('Enable TripAdvisor Widget shortcodes.', 'dmopress_textdomain'),
		)
	);
	return $available_modules;
}

function dmopress_enabled_modules(){
	$available_modules = dmopress_available_modules();
	$options = get_option('dmopress');

	$enabled_modules = array();
	
	foreach ($available_modules as $module) {
		if(array_key_exists($module['slug'], $options)){
			if($options[$module['slug']] == 'enabled'){
				array_push($enabled_modules, $module['slug']);
			}
		}
	}
